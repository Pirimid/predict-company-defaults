...
def custom_loss(model, x, y, training, curr_timeStep, final_timeStep):
    ...
    # 3. Based on whether the company went default or not decide how much weightage we should give both losses.
    if final_timeStep > -1:
        final_loss = cls_loss
        ...
        final_loss = ((1/3) * cls_loss.numpy()) + ((2/3) * reg_loss.numpy())
        return final_loss
...
def grad(model, inputs, targets, curr_timeStep, final_timeStep):
    ...
    with tf.GradientTape() as tape:
        loss_value = custom_loss(model, inputs, targets, training=True, curr_timeStep=curr_timeStep, final_timeStep=final_timeStep)
